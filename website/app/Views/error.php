<style>
        /* Override values from [site.css] */
        table { display:table; }
        table tr { background-color:initial; }
        td, th { vertical-align: initial; max-width:initial; background-color:initial; }

        /* CSS for this page */
        :root {
            --bg-image: linear-gradient(-180deg, #7481B5 0%, #4F5B93 100%);
        }
        .container { width:1200px; margin:50px auto; }
        @media screen and (max-width: 1300px) { .container { width:90%; } }
        .alert { margin-bottom: 40px; padding:20px; border:1px solid #4F5B93; border-radius: 4px; }
        .alert-danger { background-color: #4F5B93; background-image:var(--bg-image); color:#fff; }
        .alert-info { background-color:#E2E4EF; color:#4F5B93; }
        .alert, table { box-shadow:0 1px 3px 0 rgba(0, 0, 0, .3); }
        h1 { font-size:2em; }
        h2 { color:#4F5B93; margin-bottom:10px; font-size:1.75em; }
        table { text-align:left; width:100%; margin:0; margin-bottom:40px; border-collapse:collapse; }
        table thead { background-color:#4F5B93; background-image:var(--bg-image); color:#fff; }
        table > thead > tr > th { padding:12px 10px; border:1px solid #4F5B93; }
        table > tbody > tr > td { padding:8px 10px; border:1px solid #4F5B93; border-bottom:none; border-right:none; vertical-align:top; }
        table > tbody > tr:nth-child(odd) { background-color:#E2E4EF; }
